  ...
  function ($, d3, topTableData, dataWrangler) {
    ...
    TopTableView.prototype.initUI = function (root) {
      ...
      self.$root.find('#importedTable').on('scroll', function () {
        self.$root.find('#topOperations').scrollLeft($(this).scrollLeft());
      });

      self.$root.find('#topOperations').on('scroll', function () {
        self.$root.find('#importedTable').scrollLeft($(this).scrollLeft());
      });

      self.$root.find('#topOperations').on('scroll', function () {
        self.$root.find('#topLeftOperations').scrollTop($(this).scrollTop());
      });

      //this will unbind the event for reinitialization
      self.$root.find('#add-col-id-button, #copy-id-button, #remove-col-button').unbind('click');
      ...
      self.$root.find('#remove-col-button').bind('click', function (event) {
        self.oprCount++;
    ...
    TopTableView.prototype.addIDOperation = function (opr, startIndex, endIndex) {
      ...
      var dragrect = newg.append('rect')
        .attr('id', 'active')
        .attr('x', function (d) {
          return d.x = d.x + rectStartIndex * columnWidth;
        })
        .attr('height', height)
